-- Agent Memory + Long-task local state schema.
--
-- Runtime database:
--   data/state.sqlite
--
-- This file is committed as schema only. The real SQLite database is local
...
create index if not exists idx_agent_memory_links_memory
  on agent_memory_links(memory_id, link_type);
...
create index if not exists idx_agent_issues_task_status
  on agent_issues(task_id, status, severity);
...
create index if not exists idx_agent_sessions_project
  on agent_sessions(project_key, status, started_at);
...
create index if not exists idx_agent_artifacts_task
  on agent_artifacts(task_id, artifact_type, status);
